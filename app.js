...
// === Keyboard-Awareness (Mobile): hebt das Bottom Sheet über die Tastatur ===
(function enableKeyboardInset(){
  const vv = window.visualViewport || null;

  function calcKeyboardHeight(){
    if (!vv) return 0;
    // Keyboard-Höhe = innerHeight - (sichtbare Höhe + OffsetTop der VisualViewport)
    const ih   = window.innerHeight || 0;
    const used = (vv.height || 0) + (vv.offsetTop || 0);
    const kb   = Math.max(0, ih - used);
    return kb;
  }

  function applyKbInset(){
    // Nur aktiv, wenn Quick-Add offen und das Titel-Feld fokussiert ist
    const isActive = _qaOpen && (document.activeElement === qaInput);
    const kb = isActive ? calcKeyboardHeight() : 0;
    document.documentElement.style.setProperty('--kb-safe', kb > 0 ? `${kb}px` : '0px');

    // Sicherstellen, dass das Input-Feld sichtbar bleibt
    // (kleine Verzögerung, damit Layout nach bottom-Update steht)
    if (isActive) {
      setTimeout(() => {
        try { qaInput.scrollIntoView({ block:'center', inline:'nearest' }); } catch(_){}
      }, 50);
    }
  }

  // Bei Fokus/Blur des Felds einschalten/abschalten
  qaInput?.addEventListener('focus', () => {
    applyKbInset();
    vv?.addEventListener('resize', applyKbInset);
    vv?.addEventListener('scroll', applyKbInset);
  });

  qaInput?.addEventListener('blur', () => {
    document.documentElement.style.setProperty('--kb-safe', '0px');
    vv?.removeEventListener('resize', applyKbInset);
    vv?.removeEventListener('scroll', applyKbInset);
  });

  // Falls das Sheet bereits offen ist und Fokus wechselt (Edge-Cases)
  window.addEventListener('orientationchange', () => setTimeout(applyKbInset, 150));
})();
